...
function deleteEmployeeProfile(employee_id){	
   var url_loc = "index.php?app=employee_profile&cmd=delete&employee_id="+employee_id;
   window.location = url_loc;
}

function updateEmployeeProfile(employee_id){
	var ul_loc = "index.php?app=employee_profile&cmd=edit&employee_id="+employee_id; 
     ...
     if ( document.contact_form.employee_id.value == ""  )
        {
			...
			else if(document.contact_form.admin_typid.value =="")
			{
...
ajax.open('get', 'index.php?app=employee_profile&cmd=ajaxSearch&searchq='+searchq);
...
function loadPerson(){
	...
	var url='index.php?app=employee_profile&cmd=ajaxPersonLoad&ele_id='+ele_id+'&ele_lbl_id='+ele_lbl_id;
	//alert(url);
