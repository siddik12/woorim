...
var len = document.complfrm.person_id.length
	...
	else if( document.complfrm.RetItemCode.value == "" )
        {
		        alert ( "Please enter return item code" );
				RetItemCode.focus();
                valid = false;
        }

	else if( document.complfrm.return_qnt.value == "" )
        {
		        alert ( "Please enter return item quantity" );
				return_qnt.focus();
                valid = false;
        }

	else if( document.complfrm.return_amount.value == "" )
        {
		        alert ( "Please enter return amount taka" );
				return_amount.focus();
                valid = false;
        }

	else if( document.complfrm.Retinvoice.value == "" )
        {
		        alert ( "Please enter return item invoice" );
				Retinvoice.focus();
                valid = false;
        }
	else if( document.complfrm.Retsales_date.value == "" )
        {
		        alert ( "Please enter previous sales date" );
				Retsales_date.focus();
                valid = false;
        }
...
{    
	...
	//alert(itemcode);
	  if(itemcode)
	  {  var url = "?app=inv_item_sales&cmd=ajaxcheckExistingItem&itemcode="+itemcode;
...
function loadSalsManId(id){
	...
	var cmd='index.php?app=inv_item_sales&cmd=ajaxSalesManLoad&ele_id='+ele_id+'&ele_lbl_id='+ele_lbl_id;
	ajaxCall('inv_item_sales',cmd,'SlsList')
}
...
{ 
	...
	//alert(main_item_category_id);
	  ...
	  {  var url = "?app=inv_item_sales_return&cmd=ajaxModel&main_item_category_id="+main_item_category_id;
	  	//alert(url);
