...
function ajaxCall4SRBillEntry(id){
	//alert(mode);
	if(mode == 'add'){
	...
	var cmd = "ajaxInserSRtBill&slman_id="+slman_id+"&receive_amount="+receive_amount+"&receive_date="+receive_date+"&due_amount="+due_amount;
	document.getElementById('slman_id').value='';
	document.getElementById('receive_amount').value='0.00';
	...
	var receive_date = document.getElementById('receive_date').value;	
	var due_amount = document.getElementById('due_amount').value;	
	var cmd = "ajaxEditSRBill&accid="+accid+"&slman_id="+slman_id+"&receive_amount="+receive_amount+"&receive_date="+receive_date+"&due_amount="+due_amount;
...
function ajaxCall4EditSRBill(accid, slman_id, receive_amount, due_amount, receive_date){
...
function ajaxCall4DeleteSRBill(id){
	//alert(id);
	mode = 'del';
