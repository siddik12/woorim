...
function validate_4ItemIssue(){
	...
	if (document.sale.salesprice.value==''){
		alert("Please enter sales price");
		return false;
	}else if(document.sale.warranty.value=='') {
		alert("please enter Warranty");
		return false;
	}else if(document.sale.item_size.value=='') {
...
*/
	if(document.complfrm.paid_amount.value=='') {
...
{    
	...
	//alert(itemcode);
	  if(itemcode)
	  {  var url = "?app=inv_item_sales_edit&cmd=ajaxcheckExistingItem&itemcode="+itemcode;
...
/*function ajaxCall4ItemAddEdit(){
	...
	var cmd = "?app=inv_item_sales_edit&cmd=ajaxEditItemSales&sales_id="+sales_id+"&discount_percent="+discount_percent+"&salesprice="+salesprice;
alert (cmd);
...
function loadSalsManId(id){
	...
	var cmd='index.php?app=inv_item_sales_edit&cmd=ajaxSalesManLoad&ele_id='+ele_id+'&ele_lbl_id='+ele_lbl_id+'&ele_lbl_id1='+ele_lbl_id1;
	ajaxCall('inv_item_sales_emi',cmd,'SlsList')
}

function addSalsManId(customer_id, ele_id, name, ele_lbl_id, Sabek_dues, ele_lbl_id1){
		 alert(customer_id+' - '+ele_id+' - '+name+' - '+ele_lbl_id+' - '+Sabek_dues+' - '+ele_lbl_id1);
...
function searchDataPerson() {
	...
	var url='index.php?app=inv_item_sales_edit&cmd=ajaxSalesManLoad&ele_id='+ele_id+'&ele_lbl_id='+ele_lbl_id+'&ele_lbl_id1='+ele_lbl_id1+'&search='+searchq;
	//alert(url);
	ajax.open('get', url);
...
{ 
	...
	//alert(main_item_category_id);
	  ...
	  {  var url = "?app=inv_item_sales_edit&cmd=ajaxModel&main_item_category_id="+main_item_category_id;
	  	//alert(url);
