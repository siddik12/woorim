...
function validate_4ItemIssue(){
	...
	if (document.sale.customer_id.value==''){
		alert("Please select customer");
		return false;
	}else if(document.sale.salesprice.value=='') {
...
*/
	if (document.complfrm.customerID.value==''){
		alert("Please select customer");
		return false;
...
{    
	...
	//alert(itemcode);
	  if(itemcode)
	  {  var url = "?app=inv_item_sales_emi&cmd=ajaxcheckExistingItem&itemcode="+itemcode;
...
function loadSalsManId(id){
	...
	var cmd='index.php?app=inv_item_sales_emi&cmd=ajaxSalesManLoad&ele_id='+ele_id+'&ele_lbl_id='+ele_lbl_id+'&ele_lbl_id1='+ele_lbl_id1;
...
function searchDataPerson() {
	...
	var url='index.php?app=inv_item_sales_emi&cmd=ajaxSalesManLoad&ele_id='+ele_id+'&ele_lbl_id='+ele_lbl_id+'&ele_lbl_id1='+ele_lbl_id1+'&search='+searchq;
	//alert(url);
	ajax.open('get', url);
...
{ 
	...
	//alert(main_item_category_id);
	  ...
	  {  var url = "?app=inv_item_sales_emi&cmd=ajaxModel&main_item_category_id="+main_item_category_id;
	  	//alert(url);
