...
function clearValueInnerHTML(id){
	for(i=0;i<=id.length;i++){
		document.getElementById(id[i]).innerHTML="";
	}
}
function clearValueInputField(id){
	for(i=0;i<=id.length;i++){
...
function ajaxCall(app, cmd, sink){//alert (cmd);
...
var ajaxResponseReceiverArr = Array();

function ajaxCallArr(app, cmd, sink){//alert (cmd);
	document.getElementById('loaderImg').style.visibility='visible';
	ajaxResponseReceiverArr = sink;
	var url = '?app='+app+'&cmd='+cmd;
	ajax.open('get',url);
	ajax.onreadystatechange = ajaxResponseArr;
	ajax.send(null);
	
}

function ajaxResponseArr() {
	if(ajax.readyState == 4){
		var resText = Array();
		var str = ajax.responseText;
		resText = str.split('TOUPLE_SEP');
		
		for(var i = 0; i < resText.length ; i++){
		// create tr
			var receiver = ajaxResponseReceiverArr[i];
			//alert(receiver);
			if(receiver){
				document.getElementById(receiver).innerHTML = resText[i];
			}
			//alert(receiver);
		}
