...
$sql="SELECT
			...
			invoice
		FROM
			inv_item_sales where sale_pay_id='$sale_pay_id' and branch_id=$branch_id and sales_type=1 and invoice='$invoice' and sales_date='$sales_date'";
...
$item_id = $rowf['item_id'];  
            ...
            FROM
				...
				where item_id = '$item_id' and branch_id=$branch_id";//exit();
                 ...
                 if($itemCnt<=0){
                 header('location:?app=inv_item_sales_edit&salesprice='.$sales_price.'&sales_date='.$sales_date.'&invoice='.$invoice.'&sale_pay_id='.$sale_pay_id.'&warranty='.$warranty.'&msg=Not Found This Item');
                 }else{
                    $rowItem = mysql_fetch_array($resItem);
                     ...
                     //======================== Available Quantity count =====================
                        ...
                        //======================== End =====================
                        if($Stock<=0){
                            header('location:?app=inv_item_sales_edit&salesprice='.$sales_price.'&sales_date='.$sales_date.'&customer_id='.$customer_id.'&invoice='.$invoice.'&sale_pay_id='.$sale_pay_id.'&warranty='.$warranty.'&msg=Not available in stock !!');
                        }else{
                 
$sql2 = "INSERT INTO ".INV_ITEM_SALES_TBL."(item_id, sub_item_category_id, item_qnt, costprice, salesprice,sales_date,person_id,total_amount,createdby,branch_id,sales_type,sale_pay_id,invoice,sales_status,warranty)
VALUES($item_id, '$sub_item_category_id', '$item_qnt', '$cost_price', '$sales_price', '$sales_date', '$person_id', '$sales_price','$person_id','$branch_id','1','$sale_pay_id','$invoice','Not Pending','$warranty')"; //exit();
                    $res = mysql_query($sql2);
                        if($res){
header('location:?app=inv_item_sales_edit&salesprice='.$sales_price.'&sales_date='.$sales_date.'&invoice='.$invoice.'&sale_pay_id='.$sale_pay_id.'&warranty='.$warranty.'&msg=Item Saved');
                        }else{
header('location:?app=inv_item_sales_edit&salesprice='.$sales_price.'&sales_date='.$sales_date.'&invoice='.$invoice.'&sale_pay_id='.$sale_pay_id.'&warranty='.$warranty.'&msg=Not Saved');
                        }
                    }// End $Stock<=0
            }// end $itemCnt<=0
	}//eof
...
$sale_pay_id = getRequest('sale_pay_id');
 ...
 $NetSales=($ItemPrice-$TotalDisc);
				...
				$res = mysql_query($sql);
					if($res)
					{
					header('location:?app=inv_item_sales_edit&sales_date='.$sales_date.'&invoice='.$invoice.'&sale_pay_id='.$sale_pay_id.'&msg=Item Saved');
					}else{
					header('location:?app=inv_item_sales_edit&sales_date='.$sales_date.'&invoice='.$invoice.'&sale_pay_id='.$sale_pay_id.'&msg=Not Saved');
					}
...
$sale_pay_id = getRequest('sale_pay_id');
	...
	$sales_id = getRequest('sales_id');
  	if($sales_id)  {
		...
		$res = mysql_query($sql);	
		if($res){
		header('location:?app=inv_item_sales_edit&sales_date='.$sales_date.'&invoice='.$invoice.'&sale_pay_id='.$sale_pay_id.'&msg=Item Deleted');
		}else{
		header('location:?app=inv_item_sales_edit&sales_date='.$sales_date.'&invoice='.$invoice.'&sale_pay_id='.$sale_pay_id.'&msg=Not Deleted');
		}
  	}	
}  
...
$branch_id = getFromSession('branch_id');
		...
		while($row = mysql_fetch_array($res)){
		 ...
		 if($rowcolor==0){ 
			...
			$type .='<form action="?app=inv_item_sales_edit&cmd=ajaxEditItemSales" method="post" name="itmsls">
			<div><tr  class="oddClassStyle" onMouseOver="this.className=\'highlight\'" 
			onMouseOut="this.className=\''.$style.'\'" height="35">	
			<td nowrap=nowrap><a href="?app=inv_item_sales_edit&cmd=DeleteSalesItem&sales_id='.$row['sales_id'].'&invoice='.$invoice.'&sales_date='.$sales_date.'&sale_pay_id='.$sale_pay_id.'" 
	...
	$dues_amount=($afterDiscAmnt-$paid_amount);	 	
	 ...
	 $pay_type = getRequest('pay_type');
		...
		$sale_pay_id = getRequest('sale_pay_id');
			...
			$sqlDisc="UPDATE ".INV_ITEM_SALES_PAYMENT_TBL." SET total_discount ='$allDiscount', total_amount='$afterDiscAmnt',paid_amount='$afterDiscAmnt'
			   WHERE sale_pay_id ='$sale_pay_id' and branch_id=$branch_id"; 
