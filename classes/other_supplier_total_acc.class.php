<?php
class other_supplier_total_acc
{
  function run()
  {
    $cmd = getRequest('cmd');
...
$branch_id = getFromSession('branch_id');
	...
	*/
	 $query = "SELECT COUNT(distinct(supplier_ID)) as num FROM others_supplier_acc";
...
*/	
	...
	*/
	...
	{	
		...
		//previous button
		if ($page > 1) 
			$pagination.= "<a href=\"?app=other_supplier_total_acc&page=$prev\">&laquo; previous</a>";
		else
		...
		{	
			for ($counter = 1; $counter <= $lastpage; $counter++)
			{
				if ($counter == $page)
					...
					$pagination.= "<a href=\"?app=other_supplier_total_acc&page=$counter\">$counter</a>";					
			}
		}
		elseif($lastpage > 5 + ($adjacents * 2))	//enough pages to hide some
		{
			...
			if($page < 1 + ($adjacents * 2))		
			{
				...
				$pagination.= "...";
				$pagination.= "<a href=\"?app=other_supplier_total_acc&page=$lpm1\">$lpm1</a>";
				$pagination.= "<a href=\"?app=other_supplier_total_acc&page=$lastpage\">$lastpage</a>";		
			}
			//in middle; hide some front and some back
			elseif($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2))
			{
				$pagination.= "<a href=\"?app=other_supplier_total_acc&page=1\">1</a>";
				$pagination.= "<a href=\"?app=other_supplier_total_acc&page=2\">2</a>";
				$pagination.= "...";
		...
		//next button
		if ($page < $counter - 1){
			$pagination.= "<a href=\"?app=other_supplier_total_acc&page=$next\">next &raquo;</a>";
		}else{
...
function ViewTransFetch($start=null, $limit=null){
		...
		if($searchq){
 	           ...
 	           $sql='SELECT
					a.supplier_id,
					company_name,
					sum(receive_amount) as dr,
					sum(paid_amount) as cr,
				    (sum(receive_amount)-sum(paid_amount)) as balance
				FROM
					others_supplier_acc a inner join inv_supplier_info s on s.supplier_id=a.supplier_id
					where company_name LIKE "%'.$searchq.'%"  group by supplier_id order  by company_name ASC';
		}
		if($Page && !$searchq){
 	           $sql="SELECT
					a.supplier_id,
				...
				FROM
					others_supplier_acc a inner join inv_supplier_info s on s.supplier_id=a.supplier_id
					group by supplier_id order  by company_name ASC LIMIT $start, $limit";
		...
		if(!$Page && !$searchq){
				...
				FROM
					others_supplier_acc a inner join inv_supplier_info s on s.supplier_id=a.supplier_id
					group by supplier_id order  by company_name ASC	LIMIT 0, 29";
	...
	while($row = mysql_fetch_array($res)){
				...
				$html .='<tr class="oddClassStyle" onMouseOver="this.className=\'highlight\'" 
					...
					<a href="?app=other_supplier_total_acc&cmd=ViewDetailsTrans&supplier_id='.$row['supplier_id'].'">'.$row['company_name'].'</a></td>
...
FROM
	...
	$address= $row['address'];
 	            ...
 	            $sql3="select sum(receive_amount) as drAmnt, sum(paid_amount) as crAmnt
				FROM others_supplier_acc where supplier_id= $supplier_id";
		...
		require_once(OTHER_SUPPLIER_ACCOUNTS_TRANSACTION_VIEW_SKIN); 

				
	} // EOF
	
  function CollectionReport($supplier_id){
		$sql = "select transaction_date,particulars,receive_amount,paid_amount, @rt2 := @rt2 +(receive_amount-paid_amount) as balance2
 						from others_supplier_acc, (SELECT @rt2 := 0 ) as tempName2 where supplier_id=$supplier_id ";
	...
	$speci = "<table width=600 cellpadding=\"5\" cellspacing=\"0\" style='border-collapse:collapse'>
		...
		<td nowrap=nowrap style=\"border:#000000 solid 1px\">"._date($row['transaction_date'])."</td>
		<td nowrap=nowrap style=\"border:#000000 solid 1px\">".$row['particulars']."</td>
		<td nowrap=nowrap style=\"border:#000000 solid 1px\">".number_format($row['receive_amount'],2)."</td>
		<td nowrap=nowrap style=\"border:#000000 solid 1px\">".number_format($row['paid_amount'],2)."</td>
...
$res= mysql_query($sql);
	...
	while($row = mysql_fetch_array($res)){
				...
				$html .='<tr class="oddClassStyle" onMouseOver="this.className=\'highlight\'" 
				    ...
				    <td  style="border-right:1px solid #cccccc;padding:2px;">'.$i.'&nbsp;</td>
					<td style="border-right:1px solid #cccccc;padding:2px;">
					<a href="?app=other_supplier_total_acc&cmd=ViewDetailsTrans&store_id='.$row['store_id'].'">'.$row['store_name'].'</a></td>
