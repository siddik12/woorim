...
class accounts
{
  ...
  function run()
  {
     $cmd = getRequest('cmd');
      switch ($cmd)
      {
		 case 'ChallanEnterSkin'        : echo $this->ChallanEnterSkin();							break;
		 case 'ajaxInsertChallan'       : echo $this->InsertChallan();								break;
		 case 'DelChallan'       		: $screen = $this->DelChallan();   					break;
		 case 'CheckEnterSkin'          : echo $this->checkEnterSkin();								break;
		 case 'ajaxInsertCheck'         : echo $this->InsertCheck();								break;
		 case 'DelCheck'       			: $screen = $this->DelCheck();   					break;


		 case 'SrBillEntrySkin'         : echo $this->SRbillEntrySkin();							break;
		 case 'CollectionEntrySkin'     : echo $this->CollectionEntrySkin();						break;
		 case 'PendingBillSkin'         : echo $this->PendingbillEntrySkin();						break;
		 case 'ChangebillStatus'        : echo $this->ChangeBillStatus(getRequest('pending_id'));	break;
		 case 'ajaxEditPendingBill'    	: echo $this->EditpendingBill();							break;
		 case 'ajaxEditCheck'    		: echo $this->EditCheck();									break;
		 case 'ajaxEditSRCollect'    	: echo $this->EditCollection();								break;
		 case 'ajaxEditSRBill'    		: echo $this->EditSRBill();									break;
		 case 'NotPendingList'          : echo $this->notPendingListSkin();							break;
		 case 'DrVoucherEnterSkin'      : echo $this->DrVoucherEnterSkin();							break;
		 case 'CrVoucherEnterSkin'      : echo $this->CrVoucherEnterSkin();							break;
		 case 'OpeningBEnterSkin'       : echo $this->OpeningBEnterSkin();							break;
		 case 'GLreportSkin'          	: echo $this->GLReportSkin();								break;
		 case 'ajaxGLreportShow'        : echo $this->GLReportShowSkin();							break;
		 case 'GLreportShow4PDF'        : echo $this->GLReportShowSkin4PDF();					break;
		 case 'ajaxInsertDrVoucher'     : echo $this->InsertDrVoucher();								break;
		 case 'ajaxInsertCrVoucher'     : echo $this->InsertCrVoucher();								break;
		 case 'ajaxInsertOpeningB'      : echo $this->InsertOpeningBalance();						break;
		 case 'ajaxInsertBill'         	: echo $this->InsertBill();									break;
		 case 'ajaxInserSRtBill'        : echo $this->InsertSRBill();								break;
		 case 'ajaxInserCollection'     : echo $this->InsertSRCollection();							break;
		 case 'ajaxAddtoLedger'         : echo $this->AddToLedger();								break;
		 case 'delete'                	: $screen = $this->DeleteSalesmanInfo();   					break;
		// case 'delOpeningBalance'       : $screen = $this->DelOpeningBalance();   					break;
		 case 'deleteSRbill'            : echo $this->DeleteSRBill();   							break;
		 case 'Measurement'     	    : echo $this->MeasurementSkin(); 							break;
		 case 'ajaxInsertMeasurement'   : echo $this->AddSMeasurementInfo(); 						break;
		 case 'ajaxSearchMeasurement'   : echo $this->MeasurementUnitFetch();   					break;
		 case 'ajaxDeleteMeasurement'   : echo $this->DeleteMeasurement(); 							break;
		 case 'ajaxDeleteBill'   		: echo $this->DeletePendingBill(); 							break;
...
function addSalesmanInfo(){
  	 	 $info = array();
		 $info['table'] = INV_SALSEMAN_INFO_TBL;
		 $reqData =  getUserDataSet(INV_SALSEMAN_INFO_TBL);
		 $info['data'] = $reqData;
		 $info['debug']  = false;
		 $res = insert($info);
		 return $res;
...
function InsertChallan(){
 ...
 $Sql2 = "select challan_no,supplier_ID, receive_date from ".WARE_HOUSE_TBL." where challan_no = '$challan_no'";
 ...
 $supplier_ID = $row2['supplier_ID'];
		...
		echo '<b>Sorry!! already you have entered this challan no</b>&nbsp;&nbsp;&nbsp;&nbsp;<a href="?app=accounts&cmd=ChallanEnterSkin"><u>Go Back</u></a>';
			...
			echo '<b>Sorry!! not found this challan no</b>&nbsp;&nbsp;&nbsp;&nbsp;<a href="?app=accounts&cmd=ChallanEnterSkin"><u>Go Back</u></a>';
			 ...
			 $Sql3 = "SELECT sum(total_cost) as totalCost FROM view_ware_house where challan_no = '$challan_no2'";
...
*/   						  $sql = "INSERT INTO detail_account(supplier_ID,challan_no,dr,recdate)
									values('$supplier_ID','$challan_no2',$totalCost,'$receive_date')";
						$res = mysql_query($sql);
						if($res){
...
FROM
	...
	where where cr!="0.00" and challan_no LIKE "%'.$searchq.'%" ORDER BY recdate desc LIMIT 0, 29';
	}
if($Page){
 $sql="SELECT
	detail_accid,
...
FROM
	detail_account a inner join inv_supplier_info s on s.supplier_ID=a.supplier_ID
	where cr!='0.00' ORDER BY recdate desc LIMIT $start, $limit";
	}
	if(!$searchq && !$Page ){
 $sql="SELECT
	detail_accid,
...
FROM
	detail_account a inner join inv_supplier_info s on s.supplier_ID=a.supplier_ID
	where cr!='0.00' ORDER BY recdate desc LIMIT 0, 29";	
		...
		while($row = mysql_fetch_array($res)){
		 if($rowcolor==0){ 
			...
			<td><a href="?app=accounts&cmd=DelChallan&detail_accid='.$row['detail_accid'].'" onclick="return confirmDelete();" title="Delete">
			<img src="images/common/delete.gif" style="border:none"></a></td>
 ...
 function checkEnterSkin(){
	...
	/* 
	   ...
	   If you have a WHERE clause in your query, make sure you mirror it here.
	*/
	 $query = "SELECT COUNT(*) as num FROM detail_account where cr!='0.00' ";
	$total_pages = mysql_fetch_array(mysql_query($query));
	 $total_pages = $total_pages[num]; //exit();
