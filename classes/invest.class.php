...
function DeleteSupplierInfo(){
	$invest_id = getRequest('invest_id');
      if($invest_id)
      {
      	...
      	if($res)  	{   
			// echo 'deleted..';  	   
      	   header("location:?app=invest&Dmsg=Successfully Deleted");      	         	   
      	}      	
      	else
      	{ //echo 'not deleted..';  	
      		 header("location:?app=invest&Dmsg=Not Edited");      	   	
      	}      	
      }	
   }  
...
FROM
	...
	inner join settings b on b.company_id=i.company_id
	inner join share_holder s on s.holder_id=i.holder_id where invest!="0.00"
...
FROM
	...
	inner join share_holder s on s.holder_id=i.holder_id where invest!='0.00'";
		...
		$MeasurementTab = '<table width="100%" border=0 cellspacing=0 class="tableGrid">
	            <tr>
	             ...
	             <th  align="left" >Invest Amount</th>
	...
	while($row=mysql_fetch_array($res)){
				...
				onclick="javascript:updateSupplierInfo(\''.$row['invest_id'].'\',\''.
					...
					<td nowrap="nowrap">'.$row["invest"].'&nbsp;</td>
					<td nowrap="nowrap">'._date($row["invdate"]).'&nbsp;</td>
					<td>
					<a href="?app=invest&cmd=Delete&invest_id='.$row['invest_id'].'" onclick="return confirmDelete();" title="Delete">
...
function SelectBranch($branch = null){ 
		...
		$sql="SELECT company_id, company_name FROM settings";
	    $result = mysql_query($sql);
	    $branch_select = "<select name='company_id' size='1' id='company_id' style='width:215px' class=\"textBox\">";
		$branch_select .= "<option value=''>Select</option>";
			while($row = mysql_fetch_array($result)) {
			if($row['company_id'] == $branch){
			$branch_select .= "<option value='".$row['company_id']."' selected = 'selected'>".$row['company_name']."</option>";
			}
			else{
			$branch_select .= "<option value='".$row['company_id']."'>".$row['company_name']."</option>";	
			}
		}
