...
function DeleteSupplierInfo(){
	$invest_id = getRequest('invest_id');
      if($invest_id)
      {
      	...
      	if($res)  	{   
			// echo 'deleted..';  	   
      	   header("location:?app=invest_profit&Dmsg=Successfully Deleted");      	         	   
      	}      	
      	else
      	{ //echo 'not deleted..';  	
      		 header("location:?app=invest_profit&Dmsg=Not Edited");      	   	
      	}      	
      }	
   }  
...
FROM
	...
	inner join share_holder s on s.holder_id=i.holder_id where profit!="0.00"
...
FROM
	...
	inner join share_holder s on s.holder_id=i.holder_id where profit!='0.00'";
		...
		$MeasurementTab = '<table width="100%" border=0 cellspacing=0 class="tableGrid">
	            <tr>
	             ...
	             <th  align="left" >Profit Amount</th>
	...
	while($row=mysql_fetch_array($res)){
				...
				onclick="javascript:updateSupplierInfo(\''.$row['invest_id'].'\',\''.
					...
					<td nowrap="nowrap">'.$row["profit"].'&nbsp;</td>
					<td nowrap="nowrap">'._date($row["invdate"]).'&nbsp;</td>
					<td>
					<a href="?app=invest_profit&cmd=Delete&invest_id='.$row['invest_id'].'" onclick="return confirmDelete();" title="Delete">
