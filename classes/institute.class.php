  ...
  function Saveinstitute(){
	 ...
	 if($mode == 'add'){
		 ...
		 if($res['affected_rows']){
				...
				header("location:index.php?app=institute");
		 }else{
				$msg = 'UnSuccessfull !!!';
		 }
	  }
   ...
   function instituteFetch(){
	 ...
	 $searchq =$_GET['searchq'];
		if($searchq){
				 ...
				 WHERE institute_name LIKE "%'.$searchq.'%" and where stat="Shop"';
	}
	if(!$searchq){
		 ...
		 $sql='select company_id,
						...
						branch_name,
						email,
						web FROM '.SETTINGS_TBL.' i inner join branch b on b.branch_id=i.branch_id where stat="Shop"';
		 }
	$res= mysql_query($sql);
...
$branch_id = getFromSession('branch_id');
		$sql="SELECT branch_id, branch_name FROM ".BRANCH_TBL." where stat='Shop' ";
	    $result = mysql_query($sql);
	    $department_select = "<select name='branch_id' size='1' id='branch_id' style='width:220px; height:25px;' class=\"validate[required]\">";
