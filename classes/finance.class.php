  ...
  function run()
  {
     $cmd = getRequest('cmd');
...

      {
		 ...
		 case 'ReturnExpenseList'               : $screen = $this->ReturnExpenseList();   			    break;
...
********************************************************************************************************************/	
  ...
  $branch_id = getFromSession('branch_id');
	...
	/* 
	   ...
	   If you have a WHERE clause in your query, make sure you mirror it here.
	*/
	 $query = "SELECT COUNT(*) as num FROM ".DAILY_ACC_LEDGER_TBL." where branch_id=$branch_id and dr !='0.00000' and chart_id!=37 ";
	$total_pages = mysql_fetch_array(mysql_query($query));
	 $total_pages = $total_pages[num]; //exit();
   ...
   $branch_id = getFromSession('branch_id');
	 ...
	 $Page 	= getRequest('page');
		if($Page){
				...
				where dr !='0.00000' and g.branch_id=$branch_id and g.chart_id not in(27,28,29,37) order by expdate desc LIMIT $start, $limit";
	}
	else{
		 ...
		 $sql="SELECT dlygl_id,detail_accid,g.chart_id,a.account_name,expdate,particulars, dr,company_name,bank_name,account_no 
				...
				left outer join ".INV_SUPPLIER_INFO_TBL." s on s.supplier_id=g.supplier_id
				left outer join bank_info b on b.bankid=g.bankid
		where dr !='0.00000' and g.branch_id=$branch_id and g.chart_id not in(27,28,29,37) order by expdate desc LIMIT 0, 29";
		 }
	$res= mysql_query($sql);
	$html = '<table  border=0 class="tableStyleClass">
					<tr>
...
function SelectEXpCht($exp= null){ 
		$sql="SELECT chart_id,account_name FROM ".ACCOUNTS_CHART_TBL." where type='expence' and chart_id not in(35,12)";
	    $result = mysql_query($sql);
...
// ===================================================  Expence report ===========================================================
 ...
 function expencereportAll($fromexpall, $toexpall){
		 ...
		 $sql = "SELECT account_name,expdate,particulars, dr FROM ".DAILY_ACC_LEDGER_TBL." g inner join ".ACCOUNTS_CHART_TBL." a
				on a.chart_id = g.chart_id where paytype!='Bank' and dr !='0.00000' and expdate between '$fromexpall' 
				and '$toexpall' and branch_id='$branch_id' and g.chart_id not in(27,28,29,37) order by expdate desc";
 ...
 function expencereportGroup($fromexpgr, $toxpgr){
	$branch_id = getFromSession('branch_id');
			 $sql = "SELECT account_name,expdate,particulars, sum(dr) as dr FROM ".DAILY_ACC_LEDGER_TBL." g inner join ".ACCOUNTS_CHART_TBL." a
					 on a.chart_id = g.chart_id where paytype!='Bank' and dr !='0.00000' 
					 and expdate between '$fromexpgr' and '$toxpgr' and branch_id='$branch_id' and g.chart_id not in(27,28,29,37)
					 group by g.chart_id";
...
*/	$IncAllReportFetch = $this->incomeAllreport($slfrominall, $sltoinall);
 ...
 function incomeAllreport($slfrominall, $sltoinall){
		 ...
		 $sql = "SELECT account_name,expdate, cr, particulars FROM ".DAILY_ACC_LEDGER_TBL." g inner join ".ACCOUNTS_CHART_TBL." a
					 on a.chart_id = g.chart_id where cr !='0.00000' and expdate between '$slfrominall' 
					 and '$sltoinall' and branch_id='$branch_id'  order by expdate desc ";
		// }
...
$totalCr = $totalCr+$row['cr'];
 ...
 function IncomeGroupReportSkin(){
	...
	$row=mysql_fetch_array($res);
	$branch_name=$row['branch_name'];
	$IncGrupReportFetch = $this->incomeGroupreport($slfromingr, $sltoingr);
 	 require_once(INCOME_GROUP_REPORTS_SKIN);
	}
	
 function incomeGroupreport($slfromingr, $sltoingr){
...
// ================================== Bnak Deposit ========================================================================
 function BankDepoReportSkin(){
 	...
 	$bankid 	= getRequest('bankid');
	$depositReport = $this->depositReport($slfrombd, $sltobd, $bankid);
 	 require_once(BANK_DEPOSIT_REPORTS_SKIN);
	}
	
 function depositReport($slfrombd, $sltobd, $bankid=null){
 ...
 if($bankid){
		$sqlBnkDpo = "SELECT
					...
					FROM
						...
						where deposit!='0.00' and trans_date between '$slfrombd' and '$sltobd' and b.bankid='$bankid' order by trans_date DESC";
		}else{
		$sqlBnkDpo = "SELECT
						bi.bank_name,
						deposit,
					...
					FROM
						".BANK_TRANS_TBL." b inner join ".BANK_INFO_TBL." bi on bi.bankid=b.bankid 
						where deposit!='0.00' and trans_date between '$slfrombd' and '$sltobd' order by trans_date DESC ";
		}
...
//====================================================== bank Withdrow ====================================================
 function BankWithdrawReportSkin(){
	...
	$withdrawReport = $this->withdrawReport($slfrombw, $sltobw, $bankid);
 	 require_once(BANK_WITHDRAW_REPORTS_SKIN);
	}
	
 function withdrawReport($slfrombw, $sltobw, $bankid=null){
 ...
 if($bankid){	
		$sqlBnkDpo = "SELECT
					...
					FROM
						...
						on bi.bankid=b.bankid where withdrawal!='0.00' and trans_date between '$slfrombw' 
						and '$sltobw' and b.bankid=$bankid order by trans_date DESC";
			}else{
		$sqlBnkDpo = "SELECT
						bi.bank_name,
						withdrawal,
					...
					FROM
						".BANK_TRANS_TBL." b inner join ".BANK_INFO_TBL." bi 
						on bi.bankid=b.bankid where withdrawal!='0.00' and trans_date between '$slfrombw' and '$sltobw' order by trans_date DESC";
...
//====================================================== Bank Balance Sheet ====================================================
 function BankBlncShtSkin(){
	...
	$bankid 	= getRequest('bankid');
	$BankBalanceReport = $this->BankBalanceReport($slfrombw, $sltobw, $bankid);
 	 require_once(BANK_BALANCE_SHEET_REPORTS_SKIN);
	}
	
 function BankBalanceReport($slfrombw, $sltobw, $bankid=null){
 ...
 if($bankid){
 	$sqlBnkDpo = "SELECT
					...
					FROM
						".BANK_TRANS_TBL." b inner join ".BANK_INFO_TBL." bi on bi.bankid=b.bankid
						left outer join ".INV_SUPPLIER_INFO_TBL." s on s.supplier_id=b.supplier_id
						left outer join accounts_chart a on a.chart_id=b.chart_id
						left outer join branch br on br.branch_id=b.branch_id
						where trans_date between '$slfrombw' and '$sltobw' and b.bankid=$bankid order by trans_date";
				}else{
 			...
 			$sqlBnkDpo = "SELECT
					...
					FROM
						...
						left outer join accounts_chart a on a.chart_id=b.chart_id
						left outer join branch br on br.branch_id=b.branch_id
						where trans_date between '$slfrombw' and '$sltobw' order by trans_date";
	...
	$speci = "<table width=600 cellpadding=\"4\" cellspacing=\"4\" style='border-collapse:collapse'>
		...
		<td nowrap=nowrap style=\"border:#000000 solid 1px\">".$i."</td>
		<td nowrap=nowrap style=\"border:#000000 solid 1px\">".$row['bank_name']."<br>(".$row['account_no'].")</td>
		<td nowrap=nowrap style=\"border:#000000 solid 1px\">".$row['descrip']." ".$row['company_name']." ".$row['account_name']." ".$row['branch_name']."</td>
		<td nowrap=nowrap style=\"border:#000000 solid 1px\" align=right>".number_format($deposit,2)."</td>
		<td nowrap=nowrap style=\"border:#000000 solid 1px\" align=right>".number_format($withdrawal,2)."</td>
		<td nowrap=nowrap style=\"border:#000000 solid 1px\" align=center>"._date($row['trans_date'])."</td>
		</tr>";
...
FROM
	inv_item_sales s inner join inv_item_category_sub c on c.sub_item_category_id=s.sub_item_category_id
	...
	and s.branch_id=$branch_id and sales_type=1 group by s.sub_item_category_id";
	...
	$speci = "<table width=950 cellpadding=\"5\" cellspacing=\"0\" style='border-collapse:collapse'>
	            <tr>
				<th style=\"border:#000000 solid 1px\">SL</th>
				 <th nowrap=nowrap align=left style=\"border:#000000 solid 1px\">Brand</th>
		...
		<tr><td colspan='10' align='left' height='30' style='border-top:#000000 solid 1px;border-left:#000000 solid 1px;border-right:#000000 solid 1px;' >
	...
	$speci .= "<tr height=25 >
		...
		<tr height=25 >
		<td align=right nowrap=nowrap style=\"border:#000000 solid 1px\" colspan='10'> <strong>Profit : ".$profit." (".$inPercnt.")%</strong></td>
		</tr>
		<tr><td colspan='10' align='left' height='30' style='border-top:#000000 solid 1px;border-left:#000000 solid 1px;border-right:#000000 solid 1px;' >
...
FROM
	...
	inner join inv_item_category_main m on m.main_item_category_id=c.main_item_category_id
	where sales_status='Not Pending' and sales_date='$slfrom2' and pay_type='Cash'
	and s.branch_id=$branch_id and sales_type=2 group by s.sub_item_category_id";
	...
	$speci .= "<tr height=25 >
		...
		<tr height=25 >
		<td align=right nowrap=nowrap style=\"border:#000000 solid 1px\" colspan='10'> <strong>Profit : ".$profit2." (".$inPercnt2.")%</strong></td>
		</tr>";
...
FROM
	...
	$speci .= "<tr height=25 >
		<td align=right nowrap=nowrap style=\"border:#000000 solid 1px\" colspan='9'> 
...
$speci .= "<tr><td colspan='10' align='right' height='30' style='border-top:#000000 solid 1px;border-left:#000000 solid 1px;border-right:#000000 solid 1px;' ><b>&nbsp;Whole Sales Dues Received : ".number_format($cr_amountduRec,2)."</b></td></tr>
		<tr><td colspan='10' align='left' height='30' style='border-top:#000000 solid 1px;border-left:#000000 solid 1px;border-right:#000000 solid 1px;' ><b>&nbsp;Cash Received</b></td></tr>";
...
FROM
	...
	$particulars5=$row5['particulars'];
		...
		<td nowrap=nowrap style=\"border:#000000 solid 1px\" align=right colspan=7>".number_format($crAmount5,2)."</td>
		</tr>";
	...
	$speci .= "<tr height=25 >
		<td align=right nowrap=nowrap colspan=9><strong>Sub Total : </strong></td>
		<td align=right nowrap=nowrap style=\"border:#000000 solid 1px\"><strong>".number_format($totalIncome,2)."</strong></td>
		</tr>

	  <tr>
	  <td colspan='9' align='right' style=\"border-top:#000000 solid 1px\"><strong>Total Amount : </strong></td>
...
FROM
	...
	$particulars6=$row6['particulars'];
		...
		<td nowrap=nowrap style=\"border:#000000 solid 1px\" align=right colspan=7>".number_format($drAmount6,2)."</td>
		</tr>";
...
function ReturnExpenseList(){
$list=$this->ReturnExpList();
	 require_once(RETURN_EXPENSE_LIST_SKIN); 
  
}


   function ReturnExpList(){
   $branch_id = getFromSession('branch_id');
		 
		  $sql="SELECT dlygl_id,detail_accid,g.chart_id,a.account_name,expdate,particulars, dr
				FROM ".DAILY_ACC_LEDGER_TBL." g inner join ".ACCOUNTS_CHART_TBL." a on a.chart_id = g.chart_id 
				where dr !='0.00000' and g.branch_id=$branch_id and g.chart_id=37 order by expdate desc";
